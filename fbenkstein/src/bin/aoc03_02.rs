...
fn main() {
    ...
    let claims = || data.lines().map(|line| line.parse::<Claim>().unwrap());
    ...
    let mut non_intersecting_ids = BTreeSet::new();

    for claim in claims() {
        if claim
            .rect
            .cells()
            .filter_map(|cell| covered_cell_counts.get(&cell))
            .filter(|count| **count > 1)
            .count()
            == 0
        {
            non_intersecting_ids.insert(claim.id);
        }
    }

    println!("{:?}", non_intersecting_ids);
}
